...
const arrayBulan = ['Janeiro', 'Fevereiro', 'Março', 'abrill', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro']
const bulan = arrayBulan[moment().format('MM') - 1]
const config = {
    XBOT: 'bot do shaka', 
    nomer: 'wa.me/+994404918928',
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["994404918928@s.whatsapp.net"]
			const ownerNumber = ["556792545177@s.whatsapp.net"] 
			...
			switch(command) {
                ...
                case 'hidetag':
					...
					if (!isOwner) return reply('quem eh vc?')
...
case 'lofi':
                ...
                case 'reportarbug':
                    ...
                    client.sendMessage('+994404918928@s.whatsapp.net', options, text, {quoted: mek})
       ...
       client.sendMessage(from, 'Este é o número do meu criador ^-^ ',MessageType.text, { quoted: mek} )
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'clonar':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(' quem eh tu??') 
